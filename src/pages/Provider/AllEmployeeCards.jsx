...
import { GetProviderEmployeeDetail } from "../../reduxToolkit/Providers/providersApi";
import statusId from "../../hooks/statusId";
import checkStatus from "../../hooks/checkStausColor";
import genderId from "../../hooks/genderId";
const AllEmployeeCards = ({ apidata, handleCheckboxChange, selectForDelete }) => {
                            ...
                            <div className="vehicle_component">
                                ...
                                </div>
                                {
                                    item?.statusId == 3 ?
                                        ...
                                        <Link className="update_data"
                                            to="/dashboard/supplier/supplier-order-detail"
                                            onClick={() => {
                                                dispatch(GetProviderEmployeeDetail(item?.id));
                                                // dispatch(CheckProviderImage(item?.id))
                                                localStorage.setItem("provideridfordetail", item?.id)
